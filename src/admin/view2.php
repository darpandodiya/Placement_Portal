...
//		$myDateTime = DateTime::createFromFormat('Y-m-d', $_POST['DOB']);
		...
		mysqli_query($con,"UPDATE candidates SET 
                            ...
                            birthdate = '".date('d-M-Y', strtotime($_POST['DOB']))."' 
                            WHERE id = '".$id."'");		
	}
?>
<!doctype html>
<html>
	 	...
	 	if(isset($_POST['submit'])){
			if(isset($_SESSION['imgthere'])){
				...
				if ($_FILES['pic']['error'] > 0){
        	        ...
        	        if ((($_FILES['pic']['type'] == 'image/jpeg') || $_FILES['pic']['type'] == 'image/jpg') && $_FILES['pic']['size'] < 1000000 &&
					 $_FILES['pic']['size'] > 0)
        	        {
						if(file_exists('../photos/'.md5($_SESSION['username'],false) .$extension)) 
								unlink('../photos/'.md5($_SESSION['username'],false) .$extension);
								
						$_FILES['pic']['name']=md5($_SESSION['username'],false) .'.'.$extension;
	...
	?>
    		<div align='center'>
				...
				<td>
						<input type='text' name='stu_id' value='<?php echo $temp['id']; ?>' required id='stu_id'/>
				</td>
				</tr>
                ...
                <tr>
					<td class='description'><label for='cont_land'>Landline No.<b><font color='#FF0000'>*</font></b> :</label></td>
					<td>
						<input type='text' name='cont_land' value='<?php if (strlen($temp['contact_landline']) == 9) echo "0"; echo $temp['contact_landline']; ?>' placeholder='(e.g.)  0791234567' required pattern='[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]?'  id='cont_land'/>
					</td>
				</tr>
				...
				echo "<tr>
						<th>SPI</th>
						<td><input type='text' id='tabl-in' name='spi_1' value='".$temp['spi_1']."' /></td>
						<td> ".$temp['spi_2']."</td>
						<td> ".$temp['spi_3']."</td>
						<td> ".$temp['spi_4']."</td>
						<td> ".$temp['spi_5']."</td>
				...
				<tr>
                    ...
                    <td>
						...
						if($imgthere) echo "&nbsp;&nbsp;Change:";
                		?>
                    
						<input type='file' accept='image/jpeg' name='pic' <?php if(!$imgthere) echo 'required'; ?> id='pic'>
					</td>
				</tr>
	  			</tr></table></div>
      			<tr>
