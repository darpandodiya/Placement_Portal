<?php
function checkAdminUserPass($username, $password)
{
		$host = 'localhost';
		$dbusername = 'root';
		$dbpassword = '';
		...
		if (!($stmt = $mysqli->prepare('SELECT hashedpass FROM ' . $tablename . ' WHERE username = ? LIMIT 1'))) {
		...
		if (!$stmt->bind_result($hashedpass)) {
